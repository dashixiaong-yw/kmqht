...
```
[查阅知识图谱] → [修改代码(可循环)] → [lint验证] → [更新版本号] → [构建APK] → [更新知识图谱] → [同步docker-deploy] → [Git提交推送]
```
...
| 4 | 开发 | 更新版本号（含Docker BUILD_VERSION） | **先读取6处当前版本号取最大值，再+1递增**，更新6处并验证一致。**版本号确定后进入收尾构建阶段，不可回退更改** |
| 5 | 收尾 | 构建APK | **检查版本号已更新（6处一致），确认后**执行 `./gradlew assembleRelease`（签名+混淆）构建成功，**此APK为最终发布版本**（文件名含新版本号）；**失败则回到Step 2修复** |
