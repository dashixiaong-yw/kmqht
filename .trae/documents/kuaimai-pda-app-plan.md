...
### Web管理后台页面（浏览器访问 /admin）

管理员通过浏览器访问 `http://NAS_IP:8900/admin` 进入Web管理后台，API Key认证后可管理系统配置。

```
┌──────────────────────────────────────────────────────────┐
│  快麦取货通 - 管理后台                    admin ● 已认证  │
├──────────────────────────────────────────────────────────┤
│ [仪表盘] [用户管理] [拣货区管理] [快麦配置] [系统配置] [图片查看] │
├──────────────────────────────────────────────────────────┤
│                                                          │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐                 │
│  │ 取货单    │ │ 用户      │ │ 拣货区    │  ← 统计卡片    │
...
| 标签页 | 功能 | 说明 |
|--------|------|------|
| 仪表盘 | 系统概览 | 显示取货单数/用户数/拣货区数统计卡片 + 扫码配置二维码 |
| 用户管理 | 增删改查+权限 | 用户列表（用户名/权限/状态/操作），新增用户、编辑权限、启用/禁用 |
| 拣货区管理 | 增删 | 拣货区列表 + 新增输入框 + 删除按钮（二次确认） |
| 快麦配置 | 凭证管理 | 显示凭证状态（有效期/剩余天数），刷新Session按钮，手动更新凭证表单 |
| 系统配置 | API Key+服务器 | 显示当前API Key和服务器地址（SERVER_URL），只读展示 |
| 图片查看 | 按SKU查看 | 输入SKU编码查看关联的库区图/装箱图，只读不可编辑 |

> **认证机制**：页面加载时弹出输入框要求输入API Key，验证通过后显示管理内容，API Key存储在sessionStorage中。所有API请求通过X-API-Key头认证。
> **技术方案**：纯HTML+CSS+JS，内嵌到Python后端（admin.py），不引入前端框架。调用现有后端API端点。
> **App与Web权限分离**：Web后台负责系统管理（用户管理、拣货区管理、快麦凭证、服务器配置），App端负责日常业务（取货单、图片上传/删除、供应商/备注修改、扫码方式/反馈开关）。

### 扫码配置页面（浏览器访问 /setup）

管理员通过浏览器访问 `http://NAS_IP:8900/setup` 查看配置二维码，PDA扫码自动填入服务器地址和API Key。
...
> **二维码协议**：`kuaimai://setup?server=<URL>&apikey=<KEY>`，App端SetupQrParser统一解析
> **兼容格式**：同时支持纯URL格式（`http://...`），解析时自动识别
> **SERVER_URL配置**：在.env文件中设置 `SERVER_URL=http://NAS_IP:8900`，未配置时从请求Host推断
> **App端集成**：GuideScreen（首次引导）和SettingsScreen均支持扫码配置按钮，扫码后自动填入服务器地址和API Key
