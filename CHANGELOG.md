...
### 修复
- PickDetailViewModel.loadStocks() 空catch块添加Log.w，符合规则#24（禁止静默失败）
...
### 改进
- 取货单详情库区图/箱图空方块添加"库区""箱图"文字占位，无图片时更醒目
...
### 修复
- 管理后台"补全缩略图"按钮权限校验 bug：检查了不存在的 `user["roles"]` 字段，导致始终报 403。修复为使用标准权限依赖 `check_permission("settings")`
...
### 安全
- 后端 kuaimai.json 快麦凭证 AES-256-GCM 加密存储，防止服务器被入侵后凭证明文泄露
- Android 记住密码功能使用 Android KeyStore + AES/GCM 应用层加密存储
- 登录成功后立即清除内存中的密码变量

## 2.28 (2026-06-25)

### 改进
- Web管理后台文案优化：API Key→管理密码、刷新Session→刷新连接、APK管理→版本管理等，降低非技术人员理解成本
- Android端会话过期/预警提示文案优化，消除"快麦API会话"等技术术语

## 2.27 (2026-06-25)

### 改进
- 管理后台登录页输入框提示文案从"请输入API Key"改为"请输入管理密码"，降低理解成本
...
### 移除
- PDA 端：MainActivity 更新弹窗移除"在浏览器中下载"按钮（OTA已稳定，无需兜底）
- 后端: 移除 /api/app-version/qrcode 二维码下载接口
- 管理后台: 移除 APK 管理中的 PDA 扫码下载二维码
...
### 重构
- 取货单详情扫码添加商品重构：查重+占位符无锁即时反馈，API调用Mutex串行化
- 数据就绪后scrollToItem(0)固定视口，解决8-10个商品后视口偏移

### 修复
- 添加商品时HTTP 404自动重试一次，不再报错
- 重复扫码滚动索引改为filteredItems，匹配LazyColumn显示顺序

## 2.28 (2026-06-25)

### 改进
- 管理后台图片搜索Tab同时显示原图和缩略图及完整URL，便于诊断缩略图加载问题

## 2.24 (2026-06-25)

### 修复
- 取货单列表/商品详情/供应商选择等模块API 401时主动弹出重新登录弹窗

## 2.23 (2026-06-25)

### 修复
- Android 6.0 PDA OTA 安装解析错误：APK文件名改为ASCII + 复制到外部存储再安装

## 2.22 (2026-06-24)

### 修复
- 浏览器下载APK失败：后端下载端点移除publishedAt检查、Content-Disposition使用ASCII文件名+RFC 5987编码

## 2.21 (2026-06-24)

### 修复
- Android 6.0 PDA 安装APK解析错误：file:// URI 降级时增加 setReadable 设置文件全局可读

## 2.20 (2026-06-24)

### 修复
- Android 6.0 PDA OTA 安装失败：installApk 增加 file:// URI 降级

## 2.19 (2026-06-24)

### 修复
- Android 6.0 PDA OTA 安装失败：installApk 增加 file:// URI 降级 + 弹窗显示具体错误信息

## 2.18 (2026-06-24)

### 新增
- 更新弹窗内直接显示下载失败的错误详情（红色文字），用户可看到具体报错信息
...
### 修复
- 修复 main.py 未导入 KUAIMAI_CONFIG_PATH 导致启动崩溃（NameError）

## 2.13 (2026-06-24)

### 新增
- 管理后台「快麦配置」新增自动刷新诊断测试功能，一键验证 refresh_token 配置和刷新链路

## 2.12 (2026-06-24)

### 修改
- 后端启动时每次覆盖快麦凭证文件（确保每次部署都同步最新的凭证和 refresh_token）

## 2.11 (2026-06-24)

### 修复
- 后端启动时自动初始化快麦凭证文件（/data/kuaimai.json 不存在时从本地配置文件复制）

## 2.10 (2026-06-24)

### 修复
- TokenAuthenticator 遇到 401 不再错误刷新快麦 session，直接跳转登录页

## 2.9 (2026-06-24)

### 优化
- 取货单扫码添加：扫描后立即显示"添加中..."占位行，API 返回后自动替换为完整数据
- 商品详情页：立即显示条码，数据异步加载
- 图片 URL 批量预加载，减少列表滚动卡顿

## 2.8 (2026-06-24)

### 修复
- 取货单全部完成后隐藏恢复按钮，显示"已完成"提示文字
- 取货单全部完成后禁用长按删除弹窗
- 已完成取货单 24 小时自动清理（原为 30 天）

## 2.7 (2026-06-24)

### 修复
- 取货单详情页完成商品后 LazyColumn 不会跟随已完成的商品滚动到底部

## 2.6 (2026-06-24)

### 新增
- Web管理后台底部增加版本号显示：`快麦取货通 vX.X`，方便确认服务器代码版本

## 2.5 (2026-06-24)

### 新增
- 通知栏下载进度条：点击"立即更新"后通知栏显示下载百分比，关闭弹窗/回到桌面仍继续后台下载
- 下载完成通知（点击安装）：下载完成后通知栏提示"快麦取货通 vX 下载完成，点击安装更新"
- Android 13+ 通知权限请求：启动时自动请求 POST_NOTIFICATIONS 权限
- 下载结果 Toast 提示：下载完成/失败后 Toast 告知用户

### 修改
- `saveToPublicDownloads` 失败时不再影响主流程，APK 保留在应用私有目录

## 2.4 (2026-06-24)

### 修复
- 库区图/箱图缩略图无法显示：buildThumbUrl始终生成`_thumb.jpg`后缀，匹配后端统一JPEG输出
- 扫描添加新商品后自动滚动到列表顶部，不再停留在底部或不动
- 重复添加商品提示从底部Snackbar改为居中弹窗，显示✓+"该商品已在列表中"，1.5秒后自动消失

## 2.3 (2026-06-24)

### 修复
- APP更新后因旧token过期/为空导致"快麦会话已过期"弹窗——改为弹窗"登录过期"，点击"重新登录"直接跳转登录页重新登录即可恢复

### 修改
- 弹窗标题：`快麦会话已过期` → `登录过期`
- 弹窗文案：更新为"当前登录已失效，请重新登录"
- 按钮：`知道了` → `重新登录`（跳转登录页）
- 移除`前往设置`按钮
...
### 新增
- SKU图点击查看大图预览
- 库区图/箱图上传时自动生成200px缩略图（约10KB/张），减少列表加载数据传输

### 修改
- 点击供应商名称（SKU名）跳转商品详情页（原为点击SKU图跳转）
- 点击SKU图改为弹出大图预览弹窗
- 库区图/箱图列表显示改为加载缩略图，点击后显示全尺寸图片

### 修复
- 重新安装APP后首次登录点击取货列表提示"快麦会话已过期"——UserRepository内存缓存token，消除EncryptedSharedPreferences异步写入时序问题

## 1.99 (2026-06-23)

### 修复
- 普通 PDA 用户无法同步快麦凭证导致首次登录后快麦 API 全部失效——后端 `/api/kuaimai/credentials` 去掉 settings 权限要求（改为仅需登录）

## 1.98 (2026-06-23)

### 修改
- SKU图尺寸 52×52 → 90×90，垂直间距缩小，行高保持98dp

### 清理
- 删除 CoilModule.kt 遗留死代码（已改用 App.kt ImageLoaderFactory）

### 修复
- 取货单列表首次加载误报"加载失败"——删除 init 中重复 loadActiveOrders 调用消除竞态

### 流程改进
- 新增 Plan Mode 衔接规范：计划模式批准后自动转入 8 步开发流程

## 1.97 (2026-06-23)

### 修复
- Android 6.0 图片显示空白：Coil 使用系统 SSL 不信任 FRP 自签证书，改为注入 trustAll OkHttpClient

## 1.96 (2026-06-23)

### 修复
- 修复图片URL拼接缺少 `/` 分隔符导致 App 端不显示已上传的库区图/箱图
- 去掉取货单详情缩略图底部的"库区""箱图"文字标签
...
### 修复
- Android 6.0 (API 23) 兼容性修复：minSdk 从 24 降为 23，AGP 自动启用 v1 签名，解决"解析软件包时出现问题"无法安装的 bug

## 1.93 (2026-06-23)

### 修复
- 修复后台分发APK后 App 端无弹窗提示的 bug
- MainActivity 启动时检查更新从静默下载改为 AlertDialog 弹窗提示用户
...
### 新增
- 库区图和装箱图上传优先调用系统摄像头拍照，无摄像头时回退到图库选择

## 1.90 (2026-06-23)

### 修复
- 修复SKU编码大小写问题：手动输入小写编码回传备注/供应商时不再覆盖快麦原有的大写编码

## 1.89 (2026-06-23)

### 修改
- 创建用户时自动补全 settings 权限，避免管理员忘记勾选导致 PDA Worker 凭据同步失败

## 1.88 (2026-06-23)

### 修复
- 修复同步脚本：快麦凭证文件 kuaimai.json 未正确同步到 docker-deploy/data/ 子目录，导致 Docker 部署时凭证加载失败

## 1.87 (2026-06-22)

### 修改
- 重新设计确认弹窗（品牌蓝底确认按钮、增加供应商编码展示、统一样式）
- 重新设计新建取货单拣货区弹窗（原型风格2列网格布局）
- 移除引导页及相关所有引用
- 移除相机扫码功能及CameraX/MLKit依赖
- 供应商筛选按编码排序
- 规格备注修改不再需要确认弹窗，直接保存
...
- [Bug1/2] Worker 获取 SKU 数据改用后端中转（原直连快麦全部失败）
